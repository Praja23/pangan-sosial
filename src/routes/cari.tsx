...
function SearchPage() {
  ...
  const [locModal, setLocModal] = useState<
    "closed" | "consent" | "detecting" | "success" | "denied"
  >("closed");
  ...
  const items = useMemo(() => {
    ...
    if (q.trim()) {
      ...
      arr = arr.filter(
        (x) =>
          x.l.title.toLowerCase().includes(s) ||
          x.partner?.name.toLowerCase().includes(s) ||
          x.partner?.city.toLowerCase().includes(s),
      );
    }
    arr = arr.filter((x) => x.dist <= radius);
    if (sort === "expiry")
      arr.sort((a, b) => new Date(a.l.expires_at).getTime() - new Date(b.l.expires_at).getTime());
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="pt-28 pb-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6">
          <div
            className="rounded-3xl p-6 sm:p-10 relative overflow-hidden mb-6"
            style={{ background: "var(--gradient-mesh)" }}
          >
            <h1 className="font-display text-3xl sm:text-4xl font-bold">
              Cari Makanan Surplus Terdekat
            </h1>
            <p className="mt-2 text-muted-foreground">
              Aktifkan lokasi & atur radius untuk menemukan makanan layak konsumsi di sekitar Anda.
            </p>
            <div className="mt-5 grid lg:grid-cols-[1fr_auto_auto] gap-3">
              <div className="relative">
                ...
                <input
                  ...
                  placeholder="Cari menu, mitra, atau kota…"
              ...
              <select
                ...
                className="px-4 py-3 rounded-xl bg-background border border-border"
              >
            ...
            <div className="mt-4 grid sm:grid-cols-[auto_1fr_auto] gap-3 sm:items-center glass-card rounded-2xl p-3 bg-background/80">
              <button
                onClick={openLocationFlow}
                disabled={locating}
                className="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-[image:var(--gradient-warm)] text-primary-foreground text-sm font-semibold shadow-elegant hover:shadow-glow transition-shadow disabled:opacity-60"
              >
                {locating ? (
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-3 min-w-0">
                <div className="flex items-center gap-2 min-w-0">
                  ...
                  <select
                    ...
                    className="flex-1 sm:flex-initial px-3 py-2 rounded-lg border border-border bg-background text-sm min-w-0"
                  >
                    {CITIES.map((c) => (
              ...
              <span className="text-xs text-muted-foreground sm:text-right">
                {items.length} hasil dalam radius
              </span>
            </div>
          </div>
              ...
              >
                <div className="aspect-[4/3] bg-secondary overflow-hidden relative">
                  <img
                    ...
                    loading="lazy"
                  />
                  <div className="absolute top-2 left-2 px-2 py-1 rounded-full bg-background/90 backdrop-blur text-[10px] font-semibold">
                    {l.priority_label}
                  </div>
                ...
                <div className="p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between text-[11px]">
                    ...
                    <span className="inline-flex items-center gap-1 text-muted-foreground">
                      <Clock className="w-3 h-3" /> {formatCountdown(l.expires_at)}
                    </span>
                  </div>
                  <div className="mt-1 text-[11px] text-primary font-semibold inline-flex items-center gap-1">
      ...
      {locModal !== "closed" && (
        ...
        >
          ...
          <div className="relative w-full max-w-md glass-card rounded-3xl shadow-float overflow-hidden animate-scale-in">
            <button
              onClick={() => setLocModal("closed")}
              className="absolute top-4 right-4 z-10 w-8 h-8 grid place-items-center rounded-full hover:bg-secondary transition-colors"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            {locModal === "consent" && (
              <div className="p-7 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">paling dekat</span> & menghemat
                  waktu pengambilan.
                </p>
                <div className="mt-5 grid gap-2 text-left text-xs">
                  {[
                    ...
                    <div
                      key={idx}
                      className="flex gap-2 items-start p-2.5 rounded-xl bg-secondary/60"
                    >
                ...
                <div className="mt-6 grid grid-cols-2 gap-2">
                  <button
                    ...
                    className="py-2.5 rounded-xl border border-border text-sm font-semibold hover:bg-secondary transition-colors"
                  >
                    Pakai kota saja
                  </button>
                  <button
                    onClick={runDetect}
                    className="py-2.5 rounded-xl bg-[image:var(--gradient-warm)] text-primary-foreground text-sm font-semibold shadow-elegant hover:shadow-glow transition-shadow inline-flex items-center justify-center gap-1.5"
                  >
                    <Locate className="w-4 h-4" /> Izinkan
                  </button>
                </div>
              </div>
            )}
            ...
            {locModal === "detecting" && (
              <div className="p-10 text-center">
                <div className="relative mx-auto w-28 h-28 mb-6">
                  ...
                  />
                  <div
                    className="absolute inset-4 rounded-full bg-primary/40 animate-ping"
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Mohon izinkan akses lokasi di pop-up browser.
                </p>
                <div className="mt-5 inline-flex items-center gap-2 text-xs text-primary font-semibold">
                  <Loader2 className="w-4 h-4 animate-spin" /> Menyinkronkan GPS
                </div>
              </div>
            )}
            ...
            {locModal === "success" && (
              <div className="p-7 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  ...
                  <span className="font-semibold text-foreground">{detectedCity}</span>.
                </p>
                <div className="mt-4 inline-flex items-center gap-2 px-3 py-2 rounded-full bg-secondary text-xs">
                  <Building2 className="w-3.5 h-3.5 text-primary" /> Radius {radius} km •{" "}
                  {items.length} hasil
                </div>
                <button
                  onClick={() => setLocModal("closed")}
                  className="mt-6 w-full py-2.5 rounded-xl bg-[image:var(--gradient-warm)] text-primary-foreground text-sm font-semibold shadow-elegant"
                >
                  Lihat Hasil
                </button>
              </div>
            ...
            {locModal === "denied" && (
              <div className="p-7 text-center">
                ...
                <p className="mt-2 text-sm text-muted-foreground">
                  Tidak masalah — Anda tetap bisa mencari makanan menggunakan pilihan kota di bawah.
                </p>
                <div className="mt-5 grid grid-cols-2 gap-2">
                  ...
                  <button
                    onClick={useCityFallback}
                    className="py-2.5 rounded-xl bg-[image:var(--gradient-warm)] text-primary-foreground text-sm font-semibold shadow-elegant"
                  >
                    Pakai Kota
                  </button>
                </div>
